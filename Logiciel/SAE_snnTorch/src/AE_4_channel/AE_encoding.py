...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # Assuming self.encoder and self.decoder are PyTorch modules
        spk_mem_tuple = self.encoder(x)
        spk_rec, mem_rec = spk_mem_tuple[0], spk_mem_tuple[1]

        spk_mem_tuple_1 = self.decoder(spk_rec)
        spk_rec_1, mem_rec_1 = spk_mem_tuple_1[0], spk_mem_tuple_1[1]

        return mem_rec_1, spk_rec_1, mem_rec
        ...
        for epoch in range(num_epochs):
            ...
            for batch in train_set_loader:
                ...
                for window in normalized_input:
                    ...
                    #normalized_input_encode = spkg.rate(data=window, time_var_input=True)
                    window_encoded.append(normalized_input_encode)
                normalized_encoded_tensor = torch.stack(window_encoded)
                output, spk, mem_rec = net(normalized_encoded_tensor)

                # num_windows = mem_rec.size(0)
                # num_timesteps = mem_rec.size(1)
                # num_channels = mem_rec.size(2)
                # mem_rec_list.append(mem_rec.view(num_channels*num_windows, num_timesteps))
            ...
            # mem_rec_stacked = torch.cat(mem_rec_list, dim=0)
            # mem_rec_list = mem_rec_list.pop(-1)
            # mem_rec_stacked = torch.stack(tuple(mem_rec_list), dim=0)

            ### LATENT ###
            num_windows = mem_rec.size(0)
            num_timesteps = mem_rec.size(1)
            num_channels = mem_rec.size(2)
            # encoded_data_2d = mem_rec_stacked.view(num_windows, -1)
            print(mem_rec.shape)
            encoded_data_2d = mem_rec.view(num_windows*num_timesteps, num_channels)
            print(encoded_data_2d.shape)

            # encoded_data_np = torch.cat(mem_rec_list, dim=0)
            # encoded_data_np = encoded_data_np.detach().numpy()
            encoded_data_np = encoded_data_2d.detach().numpy()

            num_clusters_latent = 2
            kmeans_latent = KMeans(n_clusters=num_clusters_latent)
            kmeans_latent.fit(encoded_data_np)

            cluster_labels = kmeans_latent.labels_
            ...
            num_columns = 4
            #print(normalized_output)
            self.visualization.saveImage(tensor=normalized_encoded_tensor[-1], epoch=epoch, input=True, num_cols=num_columns, step='Train')
            self.visualization.save_encoding(tensor=normalized_input[-1], encoding=normalized_encoded_tensor[0], epoch=epoch, step='Train')
            self.visualization.saveImage(tensor=normalized_output[0], epoch=epoch, input=False, num_cols=num_columns, step='Train')
            self.visualization.saveImageCombined(input_tensor=normalized_encoded_tensor[0], output_tensor=normalized_output[0], epoch=epoch, num_cols=num_columns, step='Train')
            self.visualization.saveImageCombinedOnSame(input_tensor=normalized_encoded_tensor[0], output_tensor=normalized_output[0], epoch=1000, num_cols=num_columns, step='Train')
            self.visualization.save_subplots(input=normalized_encoded_tensor[0], output=normalized_output[0], epoch=epoch, step='Train')
            self.visualization.save_raster(data=normalized_encoded_tensor, epoch=epoch, step="Train", input=True)
            ...
            self.visualization.save_KMeans_latent(data=encoded_data_np, num_clusters=num_clusters_latent, cluster_labels=cluster_labels, epoch=epoch, step="Train", input=True)
...
if __name__ == '__main__':
    ...
    data_recordings_list = data_recordings.get('data')
    data_timesteps_dict = data_recordings.get('timesteps')
    ...
    train_set_loader, val_set_loader, test_set_loader = split_data(data_recordings_list, data_recordings_test, 0.9, 0.1, 150)
    num_inputs = 4
    hidden_layers = [16, 2]
    num_outputs = 4
    beta = 0.8
